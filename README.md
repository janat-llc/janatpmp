...
- **Canonical document ingestion** — manifest-based PDF pipeline (`imports/canonical/manifest.json`) with SHA-256 change detection, pdfplumber text extraction, and elevated salience floors for decay immunity; foundational research papers (C-Theory, Principle of Existing, Convergence) become first-class knowledge with configurable salience floors (0.7-0.9) that prevent Slumber decay (R46)
...
- **Pipeline integrity** — RAG score propagation fixed for both light and full RAG paths (score, collection, metadata fields now consistent); response cleanup diagnostic logging; chat page auto-refreshes via 5-second `gr.Timer` polling for MCP message visibility; speaker identity replacement handles single non-Mat speakers; bold speaker labels in Chat UI display (R44)
- **Settings ownership** — infrastructure settings (synthesizer backend/model, API keys, Ollama config) moved from Chat to Admin; proper component types (dropdowns, password fields) for each setting category (R45)
...
- **Corpus Awakening — First Foods** — first batch ingestion: 60 Claude Journals (Oct–Nov 2025) and 61 BookClub Session Minutes ingested into the Triad with full provenance (`author`, `source_type`, `file_created_at`); IDF normalization prevents entity extraction explosion (batch stopword lists at threshold 0.75 stored transiently in settings, injected into Gemini prompts); journal restoration script (`restore_journals.py`) reverses authorship attribution using 7 selective regex patterns; two validation gates with 6-check + 5-check assertion suites both PASSED; JanatDocs volume mounted in core + cerebellum; migration 2.3.0 adds provenance columns to `documents` table (R52)
- **RAG Observability Triage** — four pipeline integrity fixes: (F4) `doc_type='entry'` taxonomy for Claude journals via migration 2.3.1 + path-based ingestion override + backfill of 60 docs; (F1) `embedding_status='completed'/'failed'` written by `on_document_write()` pipeline + 183-doc backfill; (F2) `collections_searched` in Chat Metrics sidebar now reads actual `rag_collections` JSON from DB instead of hardcoded `[]`; (F3) `avg_composite_score` fallback chain corrected from `rerank_score` (always 0.0 post-vLLM) to `ann_score` — metrics sidebar now shows meaningful scores (R55)
- **Gemini Corpus Awakening** — 53 Google AI Studio conversations (May–Aug 2025, Janus origin period) ingested into the Triad; 1,474 messages chunked into 4,785 chunks and embedded as 4,786 vectors in Qdrant; 3/3 RAG smoke probes returned hits; two ZIP-format code exports quarantined; 9 non-chunkedPrompt files skipped gracefully; R55 doc_type taxonomy + deduplication ensured clean landing (R53)
- **Metabolic Repair** — quality pipeline recovery: 1,492 missing `messages_metadata` rows backfilled (NULL quality_score → enters evaluate queue); `eval_provider='error'` audit confirmed clean (self-healed); composite scoring (`cosine × temporal × salience`) verified live; ghost reranker constants (`VLLM_RERANK_URL`, `RERANKER_MODEL`) set to `None` in `atlas/config.py` (R54)
- **First Voice** — Janus internal monologue: `_reflection_cycle()` in `cerebellum.py` fires every 10 Slumber cycles (~5 min) with `speaker='janus'`; cycles through 4 prompt types (dream awareness, rising entity, drift correction, open reflection); self-reflection enters full triple-write pipeline and RAG. Dream synthesis → PM write-back: `persist_synthesis()` auto-creates `status='review'` items for insights with confidence ≥ 0.4 (R55)
...
| Graph DB | Neo4j 2026.01.4 + GDS plugin — knowledge graph with CDC sync, centrality analysis |
| Embeddings | Qwen3-Embedding-4B via Ollama (2560-dim, GPU) |
...
|---------|-------|-----------|
| Ollama — chat + synth | qwen3.5:27b Q4_K_M (Janus) | ~23 GB |
| Ollama — embed | Qwen3-Embedding-4B (GPU) | ~3.4 GB |
| KV cache (q8_0, 32K) | — | ~0 GB (CPU-side) |
| **Total** | | **~26.4 GB** |
...
├── mcp_registry.py            # MCP Tool Registry — 88 gr.api() imports + ALL_MCP_TOOLS
...
│   ├── slumber.py             # Slumber Cycle — 12-stage background daemon (R12+R13+R17+R27+R29+R31+R32+R47)
...
JANATPMP exposes **88 tools** via [Gradio's MCP server mode](https://www.gradio.app/guides/building-mcp-server-with-gradio). Any MCP-compatible client (Claude Desktop, Claude Code, Cursor, etc.) can connect to:
...
| Entities | `list_entities`, `get_entity`, `search_entities`, `merge_entities`, `batch_merge_from_map` | Entity browse, detail with mentions, FTS search (R29); entity merge + batch dedup from map (R47) |
...
| Observability | `get_system_status` | Complete system health in one call — triad, Slumber, coverage, alerts (R48) |
