...
![MCP](https://img.shields.io/badge/MCP-87%20Tools-blueviolet)
...
- **87 MCP tools** for AI assistant integration (items, tasks, documents, domains, conversations, relationships, vectors, graph, telemetry, ingestion, chunks, entities, entity merge, Janus lifecycle, backups, file registry, temporal context, semantic edges, dream synthesis, backfill orchestration, knowledge state, register mining, sprint view)
...
├── mcp_registry.py            # MCP Tool Registry — 87 gr.api() imports + ALL_MCP_TOOLS
...
JANATPMP exposes **87 tools** via [Gradio's MCP server mode](https://www.gradio.app/guides/building-mcp-server-with-gradio). Any MCP-compatible client (Claude Desktop, Claude Code, Cursor, etc.) can connect to:
