-- Migration 2.3.1: Add 'entry' to doc_type CHECK constraint (R55)
-- 'entry' = personal journal entries (Claude journals, future Janus journals)
-- WARNING: 'journal' is RESERVED — never use as a doc_type value
--          (conflicts with JIRI Journal, ISSN 3070-9288)
--
-- SQLite has no ALTER TABLE MODIFY CHECK — full table recreation required.
-- This migration also drops and recreates all 7 triggers on documents
-- (FTS sync, timestamp, CDC) so full-text search and graph sync remain intact.
...
-- Step 2: Drop FTS table (it references documents via triggers)
DROP TABLE IF EXISTS documents_fts;

-- Step 3: Recreate documents with 'entry' added to doc_type CHECK
...
-- Step 4: Copy all data (column-explicit to handle legacy extra columns in documents_old)
...
SELECT id, doc_type, source, title, content, author, speaker,
    source_type, file_created_at, file_path, conversation_uri, embedding_status,
    NULL, 0.0, created_by, modified_by, created_at, updated_at
FROM documents_old;
...
VALUES ('2.3.1', 'Add entry to doc_type CHECK constraint');
