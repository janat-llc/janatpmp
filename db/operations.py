...
def init_database():
    ...
    with get_connection() as conn:
        ...
        if cursor.fetchone() is None:
            migration_path = Path(__file__).parent / "migrations" / "1.9.0_speaker_identity.sql"
            if migration_path.exists():
                conn.executescript(migration_path.read_text(encoding="utf-8"))
        ...
        # WARNING: 'journal' is RESERVED as doc_type (conflicts with JIRI Journal, ISSN 3070-9288)
        if conn.execute(
            "SELECT version FROM schema_version WHERE version='2.3.1'"
        ).fetchone() is None:
            migration_path = Path(__file__).parent / "migrations" / "2.3.1_entry_doctype.sql"
            if migration_path.exists():
                conn.executescript(migration_path.read_text(encoding="utf-8"))
